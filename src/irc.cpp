...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #bitbixTEST2\r");
            Send(hSocket, "WHO #bitbixTEST2\r");
        } else {
            // randomly join #bitbix00-#bitbix05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bitbix%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bitbix%02d\r", channel_number).c_str());
        }
