...
Alternatively, install Qt Creator and open the `bitbix-qt.pro` file.

An executable named `bitbix-qt` will be built.
...
A warning for people using the *static binary* version of Bitbix on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Bitbix is linked against libdb5.3.
...
installed causes bitbix-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
